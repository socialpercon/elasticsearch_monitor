...
EXECUTIVE_FILE_PATH="${LOGSTASH_HOME}/bin/logstash"
...
function start() {
    ...
    if [[ -f "${PID_FILE}" ]]; then
        ...
        if kill -0 ${pid} >/dev/null 2>&1; then
            action_msg "logstash already running" "${SET_PASSED}"
            return 0;
        fi
    fi
    ...
    init_default_dirs
    printf "\n\n\n\n\n" >> ${LOG_FILE}
    nohup nice -n 0 ${EXECUTIVE_FILE_PATH} -f ${BIN}/logstash-grok.conf >> "${LOG_FILE}" 2>&1 < /dev/null &
    pid=$!

    if [[ -z "${pid}" ]]; then
        action_msg "logstash start" "${SET_ERROR}"
        return 1
    else
        action_msg "logstash start" "${SET_OK}"
        echo ${pid} > ${PID_FILE}
    fi
}
